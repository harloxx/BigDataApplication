   ...
   <div class="container">
     ...
     <?php
            ...
            if(mysqli_connect_error()){
                ...
                $gnr = $_POST['genre_id'];
                $sql_update = "UPDATE Genre_Film
                SET genre_id = (SELECT id FROM Genre WHERE name=\"장르 정보 없음\")
                WHERE genre_id = $gnr";
                $res_update = mysqli_query($mysqli, $sql_update);

                if($res_update){
                    $sql_delete = "DELETE FROM Genre WHERE id=$gnr";
                    $res_delete = mysqli_query($mysqli, $sql_delete);
                    if($res_delete){
                        echo "<div>장르 삭제에 성공했습니다.</div>";
                    }
                    else{
                        echo("<div>장르 삭제에 실패했습니다.</div>");
                    }
                }else{
                    echo("<div>장르 삭제에 실패했습니다.</div>");
                }
